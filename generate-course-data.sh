...
for i in AAAD AERO AHEC AHSC AMST ANES ANTH APPL APSM ANES ANTH APPL APSM ARAB ARCH ARMY ARTH ARTS ASHV ASIA ASTR \
...
do
cat $DIRECTORY_OF_CLASSES | grep -A 17 "$i       [0-7].*" |./generate_json $i >> $COURSES_UNFINALIZED
done
