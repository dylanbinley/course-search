...
//    printf("%s\n", "[");
    ...
    while(fgets(string, 1000, stdin) != NULL){
        ...
        if (!isRecitation && !hasTeacher && (string[0] == 'I' || string[0] == 'G' || compareStrings(string, "TA", 2) || 
                    compareStrings(string, "RA", 2) || compareStrings(string, "DEAN", 4) )){ 
            //    printf("%s",string);
            printTeacher(string);
            hasTeacher = true;
        }
...
void printEnrollmentInfo(char* string){
    ...
    i = skipToNumber(string, i);
    i = printMember(string, "waitlist_capacity", i);

    //get waitlist total on next line
    char c;
